...
// 日付ごとの最終更新時刻を保存（メモリ内）
const lastFetchTime: Map<string, Date> = new Map();
 ...
 */
function checkRateLimit(date: string): Date | null {
 ...
 * 最終更新時刻を記録
 */
function recordFetchTime(date: string): void {
 ...
 * 指定日の最終更新時刻を取得
 */
function getLastFetchTime(date: string): Date | null {
  ...
  try {
    ...
    const anaStr =
      Array.isArray(ana) && ana.length > 0
        ...
        : null;
    const jikuStr =
      Array.isArray(jiku) && jiku.length > 0
        ? jiku.filter((n: number) => typeof n === 'number').join(',')
        : null;
    const osaeStr =
      Array.isArray(osae) && osae.length > 0
        ? osae.filter((n: number) => typeof n === 'number').join(',')
        : null;
