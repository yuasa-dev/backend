...
function detectProvider(databaseUrl: string): 'sqlite' | 'postgresql' {
...
function generateDatasourceBlock(provider: 'sqlite' | 'postgresql'): string {
...
function main() {
  ...
  if (!databaseUrl) {
    ...
    console.error('  SQLite:     DATABASE_URL="file:./data.db"');
